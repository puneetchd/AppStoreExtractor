...
/**
 * SAProgressHUD operation modes.
 */
typedef enum {
    /** Progress is shown using an UIActivityIndicatorView. This is the default. */
    SAProgressHUDModeIndeterminate,
    /** Progress is shown using a SARoundProgressView. */
	SAProgressHUDModeDeterminate,
	/** Shows a custom view */
	SAProgressHUDModeCustomView
} SAProgressHUDMode;


/**
 * Defines callback methods for SAProgressHUD delegates.
 */
@protocol SAProgressHUDDelegate <NSObject>
 ...
 */
@interface SARoundProgressView : UIProgressView {
 ...
 * The SAProgressHUD window spans over the entire space given to it by the initWithFrame constructor and catches all
 ...
 * - SAProgressHUDModeIndeterminate - shows a UIActivityIndicatorView
 * - SAProgressHUDModeDeterminate - shows a custom round progress indicator (SARoundProgressView)
 * - SAProgressHUDModeCustomView - shows an arbitrary, user specified view (@see customView)
 *
 ...
 */
@interface SAProgressHUD : UIView {
	
	SAProgressHUDMode mode;
	...
	id<SAProgressHUDDelegate> delegate;
	NSString *labelText;
...
/**
 * The UIView (i.g., a UIIMageView) to be shown when the HUD is in SAProgressHUDModeCustomView.
 * For best results use a 37 by 37 pixel view (so the bounds match the build in indicator bounds). 
 */
@property (retain) UIView *customView;

/** 
 * SAProgressHUD operation mode. Switches between indeterminate (SAProgressHUDModeIndeterminate) and determinate
 * progress (SAProgressHUDModeDeterminate). The default is SAProgressHUDModeIndeterminate.
 */
@property (assign, nonatomic) SAProgressHUDMode mode;

/** 
 * The HUD delegate object. If set the delegate will receive hudWasHidden callbacks when the HUD was hidden. The
 * delegate should conform to the SAProgressHUDDelegate protocol and implement the hudWasHidden method. The delegate
 * object will not be retained.
 */
@property (assign) id<SAProgressHUDDelegate> delegate;
