...
- (void)setMode:(SAProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == SAProgressHUDModeDeterminate) {
...
- (void)updateProgress {
    [(SARoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == SAProgressHUDModeDeterminate) {
        self.indicator = [[[SARoundProgressView alloc] initWithDefaultSize] autorelease];
    }
    else if (mode == SAProgressHUDModeCustomView && self.customView != nil){
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"SAProgressHUDViewIsNillException" 
					format:@"The view used in the SAProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.mode = SAProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)done {
    ...
    if(delegate != nil && [delegate conformsToProtocol:@protocol(SAProgressHUDDelegate)]) {
...
- (void)fillRoundedRect:(CGRect)rect inContext:(CGContextRef)context {
    ...
    CGContextSetRGBFillColor(context, 0.0,0.0,0.0, self.opacity);
...
@implementation SARoundProgressView
